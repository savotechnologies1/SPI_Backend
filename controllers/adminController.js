...
const sendSupplierEmail = async (req, res) => {
  try {
    ...
    console.log('supplierEmail',templateData,supplierEmail)
...
const customOrderSchedule = async (req, res) => {
  try {
    ...
    const allPrismaPromises = [];
    const customOrderIdsToUpdate = new Set();
    const submittedBy =
      req.user.role === "superAdmin"
        ? { submittedByAdmin: { connect: { id: req.user.id } } }
        : { submittedByEmployee: { connect: { id: req.user.id } } };
...
const timeClockList = async (req, res) => {
  try {
    ...
    const formattedData = allProcess.map((item) => {
      ...
      return {
        id: item.id,
        name: `${item.employeeInfo?.firstName || ""} ${item.employeeInfo?.lastName || ""
          }`,
...
const dashBoardData = async (req, res) => {
  try {
    ...
    const scrapFromProduction = productionResponses.reduce((sum, record) => {
      ...
      const cost = parseFloat(record.PartNumber?.cost || 0);
      return sum + (qty * cost);
    }, 0);
...
const dailySchedule = async (req, res) => {
  try {
    ...
    const whereClause = {
      ...
      OR: [
        { delivery_date: rangeFilter },
        { order_date: rangeFilter },
        {
          StockOrder: {
            OR: [
              { shipDate: rangeFilter },
...
//     // 1. Fetch Existing Pending Stock Orders (Jo aapne form se banaye hain)
//     const stockOrders = await prisma.stockOrder.findMany({
//       where: {
//         isDeleted: false,
//         status: { in: ["Pending", "Pending Payment"] }, // Sirf wahi jo pay nahi huye
//         OR: searchQuery ? [
//           { orderNumber: { contains: searchQuery } },
//           { customerName: { contains: searchQuery } }
//         ] : undefined
...
//     // 2. Fetch Custom Orders (Ye bhi pehle se database mein hain)
//     const customOrders = await prisma.customOrder.findMany({
//       where: {
//         isDeleted: false,
//         status: "Pending",
...
//       success: true,
//       data: {
//         // Frontend ko hum ek hi format mein bhejenge
//         catalogProducts: stockOrders.map(so => ({
//           id: so.id,
//           orderNumber: so.orderNumber, // Original Number
//           name: `Order: ${so.orderNumber}`,
//           description: `Product: ${so.productNumber}`,
//           price: parseFloat(so.totalCost) / so.productQuantity, // Unit price
//           totalAmount: parseFloat(so.totalCost),
...
//     // 1. Fetch Unpaid Stock Orders (paymentStatus check added)
//     const stockOrders = await prisma.stockOrder.findMany({
//       where: {
//         isDeleted: false,
//         paymentStatus: "PENDING", // 👈 IMPORTANT: Only show unpaid orders
//         OR: searchQuery ? [
//           { orderNumber: { contains: searchQuery } },
//           { customerName: { contains: searchQuery } }
//         ] : undefined
...
//     // 2. Fetch Unpaid Custom Orders (paymentStatus check added)
//     const customOrders = await prisma.customOrder.findMany({
//       where: {
//         isDeleted: false,
//         paymentStatus: "PENDING", // 👈 IMPORTANT: Only show unpaid orders
...
//     // 3. Dynamic Tenant Gateway fetch karein
...
//           name: `Order: ${so.orderNumber}`,
//           description: `Product: ${so.productNumber}`,
//           price: parseFloat(so.totalCost) / (so.productQuantity || 1), 
//           totalAmount: parseFloat(so.totalCost),
//           customerName: so.customerName,
//           customerEmail: so.customerEmail || "", // Frontend auto-fill ke liye
//           type: 'STOCK'
//         })),
//         readyCustomOrders: customOrders.map(co => ({
//           id: co.id,
//           orderNumber: co.orderNumber,
//           customerName: co.customerName,
//           customerEmail: co.customerEmail || "", // Frontend auto-fill ke liye
//           totalCost: parseFloat(co.totalCost),
//           type: 'CUSTOM'
//         })),
//         settings: { 
//           // Database se active gateway uthayein
//           activeGateway: admin?.tenant?.activeGateway || "STRIPE" 
//         }
//       }
//     });
//   } catch (error) {
...
const getOrderCatalogData = async (req, res) => {
    try {
        ...
        const bomProducts = await prisma.partNumber.findMany({
            where: {
                isDeleted: false,
                components: { some: {} }, 
                OR: searchQuery ? [
        ...
        return res.status(200).json({
            success: true,
            data: {
                // BOM based Products for Catalog
                catalogProducts: bomProducts.map(p => ({
                    id: p.part_id,
                    name: p.partNumber,
                    description: p.partDescription || "BOM Product",
                    price: parseFloat(p.cost) || 0,
                    type: 'BOM_PRODUCT'
                })),

                // Custom Orders for "Ready to Sell" section
                readyCustomOrders: customOrders.map(co => ({
                    id: co.id,
                    orderNumber: co.orderNumber,
                    customerName: co.customerName,
                    customerEmail: co.customerEmail || co.customer?.email || "",
