...
const getScheduleProcessInformation = async (req, res) => {
  try {
    ...
    // The rest of the logic can now proceed with the correctly identified 'nextJob'.
    const { order_id, part_id } = nextJob;

    // Fetch related production data in parallel for better performance
    const [lastProductionCycle, currentProductionResponse, upcomingOrder] =
      await Promise.all([
        // Gets the last cycle details for this station (employee, time)
        ...
        // REFACTOR 3: A more robust query for the "next" job at this station
        prisma.stockOrderSchedule.findFirst({
          where: {
            processId,
            id: { not: nextJob.id }, // Not the current job
            status: { in: ["new", "progress"] },
            createdAt: { gte: nextJob.createdAt },
          },
          include: { order: { select: { shipDate: true } } },
          orderBy: { createdAt: "asc" },
        }),
      ]);

    let scheduleQuantity;
    if (currentProductionResponse?.scheduleQuantity) {
      scheduleQuantity = currentProductionResponse.scheduleQuantity;
    } else if (nextJob.type === "part") {
      scheduleQuantity =
        (nextJob.quantity || 0) * (nextJob.part?.minStock || 1);
    } else {
      scheduleQuantity = nextJob.order?.productQuantity || 0;
    ...
    const responseData = {
      ...
      // Use completed quantity from the schedule itself, which should be the source of truth
      completedQty: nextJob.completedQuantity || 0,
      scheduleQuantity: scheduleQuantity,
      // remainingQty: remainingQty > 0 ? remainingQty : 0, // Ensure remaining is not negative
      // REFACTOR 5: BUG FIX - Use optional chaining to prevent crash
